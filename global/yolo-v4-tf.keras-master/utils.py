...
def read_annotation_lines(annotation_path, test_size=None, random_seed=5566):
    with open(annotation_path, encoding='ISO-8859-1') as f:
        lines = f.readlines()
    if test_size:
...
class DataGenerator(Sequence):
    ...
    def __init__(self,
        ...
        self.num_classes = len([line.strip() for line in open(class_name_path, encoding='ISO-8859-1').readlines()])
    ...
    def get_data(self, annotation_line):
        ...
        if self.augmentation:            
            # experimental, no idea if this works...
            box_list = [ BoundingBox(x1 = b[0], x2 = b[2], y1 = b[1], y2=b[3], label = b[4]) for b in boxes ]
            ...
            img = image_aug
            #'2019-11-15_Gnu_Landau_2_026751.jpg', '396,31,929,508,0', '396,31,939,508,1'
            tmp_boxes = [str(bb.x1)+','+str(bb.y1)+','+str(bb.x2)+','+str(bb.y2)+','+str(bb.label) for bb in bbs_aug.bounding_boxes]
            boxes = np.array([np.array(list(map(float, box.split(',')))) for box in tmp_boxes], dtype=np.float32)
