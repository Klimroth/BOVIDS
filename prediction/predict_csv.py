...
from configuration import get_postprocessing_rule as get_postprocessing_rule
...
def predict_per_night(gpu_code = GPU_TO_USE,
    ...
    csv_file_info = _read_csv_file_per_line()

    for csv_line_number in sorted(csv_file_info.keys()):
        csv_current_date = csv_file_info[csv_line_number]['date']
        csv_current_enclosure = csv_file_info[csv_line_number]['enclosure_code']
        csv_current_individualcodes = csv_file_info[csv_line_number]['individual_codes']
        print('*****************************************************************')
        print("*****************************************************************")
        print(datetime.now().strftime('%Y-%m-%d %H:%M'))
        print("Line: ", csv_line_number)
        print("Enclosure: ", csv_current_enclosure)
        print("Date: ", csv_current_date)
        print("*****************************************************************")
        print("*****************************************************************")
        ...
        if not skip_image_creation:
            ...
            print('**************************************************************')
            image_creation_functions.generate_raw_images(csv_file_line = csv_line_number)
        ...
        if not skip_individual_detection:
            ...
            image_cutout_functions.predict_multiple_nights(enclosure_info, individual_info, 
                                                           required_enclosure = csv_current_enclosure,
                                                           required_date = csv_current_date)
        ...
        if not skip_single_frame_behavior_total:
            ...
            for individual_code in sorted(individual_info.keys()):
                ...
                for datum in individual_info[individual_code]['dates']:
                    ...
                    if not individual_code in csv_current_individualcodes:
                        continue
                    if not datum == csv_current_date:
                        continue
                ...
                if not skip_removing_temporary_files:
                    for enclosure_code in sorted(enclosure_info.keys()):
                        for datum in enclosure_info[enclosure_code]['dates']:
                            ...
                            if not enclosure_code == csv_current_enclosure:
                                continue
                            if not datum == csv_current_date:
                                continue
            ...
            for individual_code in sorted(individual_info.keys()):
                ...
                for datum in individual_info[individual_code]['dates']:
                    ...
                    post_processing.post_process_night(single_frame_csv = sf_csv, 
                                                   ...
                                                   truncation_areas = individual_info[individual_code]['truncation_areas'][index],
...
def _read_csv_file_per_line(csv_filepath = configuration.INPUT_CSV_FILE):
    ...
    with open(csv_filepath) as csv_file:
        ...
        for row in csv_reader:
            ...
            if line_count == 0:
                if len(row) < 12:
                    print("Overview file has the wrong format.", csv_filepath)
                    return False, False
                else:
                    line_count += 1
                    continue
            ...
            ret[line_count] = {}
            ret[line_count]['enclosure_code'] = enclosure_code
            ret[line_count]['individual_codes'] = individual_codes
            ret[line_count]['date'] = _correct_date(str(row[0]))
...
def _get_enclosure_and_individuals(csv_filepath = configuration.INPUT_CSV_FILE):
    ...
    if not os.path.exists(csv_filepath):
        print("Error: Input-CSV-file was not found.", csv_filepath)
        return False, False
        ...
        for row in csv_reader:
            ...
            if not enclosure_code in enclosure_info.keys():
                enclosure_info[enclosure_code] = {'dates': [], 'individuals': [], 'start_times': [], 'end_times': [], 
                                                  'truncations': [], 'truncation_areas': [], 'postprocessors': [], 'postprocessors-binary':[]}
            ...
            enclosure_info[enclosure_code]['truncations'].append( {'up': int(row[8]), 'bot': int(row[9]), 'left': int(row[10]), 'right': int(row[11])} )
            
            if len(row) >= 13:
                # truncations have the format x0*y0-x1*y1-...-xn*yn;
                for area in row[12].split(';'):   
                    if len(area) > 0:
                        enclosure_info[enclosure_code]['truncation_areas'].append( np.array([ [int(p.split('*')[0]), int(p.split('*')[1])] for p in area.split('-')   ]) )
                    else:
                        enclosure_info[enclosure_code]['truncation_areas'].append( [] )
            else:
                enclosure_info[enclosure_code]['truncation_areas'].append( [] )
            ...
            for individual_code in individual_codes:
                ...
                individual_info[individual_code]['truncations'].append( {'up': int(row[8]), 'bot': int(row[9]), 'left': int(row[10]), 'right': int(row[11])} )

                
                if len(row) >= 13:
                    for area in row[12].split(';'):     
                        if len(area) > 0:
                            individual_info[individual_code]['truncation_areas'].append( np.array([ [int(p.split('*')[0]), int(p.split('*')[1])] for p in area.split('-')   ]) )
                        else:
                            individual_info[individual_code]['truncation_areas'].append( [] )
                else:
                    individual_info[individual_code]['truncation_areas'].append( [] )
                    
                individual_info[individual_code]['postprocessors'].append(get_postprocessing_rule(individual_code, mode = 0))
                individual_info[individual_code]['postprocessors-binary'].append(get_postprocessing_rule(individual_code, mode = 1))
